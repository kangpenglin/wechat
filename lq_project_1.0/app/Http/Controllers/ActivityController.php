...
//        $capture = $request->input("capture");
    ...
    public function getAllActivity()
    {
        ...
        $participate_list = DB::select("select * from activity where activity_id  in (select activity_id from user_acti where wechat_id=?)", [$wechat_id]);       if (count($participate_list) != 0 | count($no_participate_list) != 0) {
            $code = 200;
